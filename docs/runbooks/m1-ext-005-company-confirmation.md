...
3. 只填写脱敏元数据：确认编号、带时区的确认时间、授权人员不透明引用、公开对客字段、打码客服展示值和三类资料的受控存储状态。
   - 有真实内部引用时使用 `REFERENCE_PROVIDED`，并填写不含访问令牌的 `vault://controlled/...`、`dms://controlled/...` 或 `object://controlled/...` 引用。
   - 资料确实位于受控存储但没有内部编号时，使用 `NO_INTERNAL_IDENTIFIER` 且将 `reference` 保持为 `null`；不得为通过校验编造编号。
...
- 如有原件引用，只允许 `vault://controlled/...`、`dms://controlled/...` 或 `object://controlled/...`，且不得包含访问令牌；没有内部编号时必须明确记录 `NO_INTERNAL_IDENTIFIER`。
