...
- 微信退款适配器现在显式取得服务端原交易 `PaymentTransaction.amount` 作为必填原订单金额；仓储要求它与订单 `cashAmount` 一致，服务在通道认领前校验其为正整数且不小于本次微信退款金额。该内部金额不接受客户端输入，也不进入响应 DTO。
- 原福利卡账户或微信原交易字段/金额异常时，在通道认领和外呼前以 `REFUND_ALLOCATION_INVALID` 失败关闭；重复请求保持同样错误，不会把通道遗留为 `PROCESSING`。
