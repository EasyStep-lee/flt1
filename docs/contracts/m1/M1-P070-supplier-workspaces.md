...
- 负例：
  - NEG-M1-070-01：跨职能路由或模块访问失败关闭；
  - NEG-M1-070-02：客户端跨供应商或归属覆盖字段在查询前被拒绝；
  - NEG-M1-070-03：八类页面不得合并角色、菜单或模块目录；
  - NEG-M1-070-04：职能切换后旧会话立即失权且不能复用。
  - NEG-M1-070-05：较旧的筛选或路由响应不得覆盖最新请求的页面状态。
