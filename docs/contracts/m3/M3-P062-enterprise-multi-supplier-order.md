...
- 网络未知结果保留采购车和与当前 `skuId + quantity` 标准签名绑定的原幂等键；相同购物车重试复用原键，购物车内容变化必须生成新键，避免异体请求陷入 `IDEMPOTENCY_CONFLICT`；仅确认 API 返回成功后清空采购车和命令键。
...
- 页面行为：三个供应商商品可形成统一采购车、统一结算、一次提交和一个公司订单结果；同体未知结果复用原键、改变购物车后换键；响应及页面源码无供应价泄露。
