...
const updatedAt = '2026-08-14T05:49:12Z';
...
await updateCsv('03-任务台账.csv', (row) => {
  ...
  if (row.TaskID === 'M3-P024') return {
    ...
    Branch: 'codex/m3-payment-idempotency', CommitSHA: '', PullRequest: '', CI: 'NOT_EXECUTED',
    UpdatedAt: updatedAt,
    Notes: 'RED：API构建通过但预支付端点404。GREEN：Prisma仓储3/3、API4/4、迁移1/1、P0 E2E1/1；MySQL空库/升级/恢复dry-run通过；OpenAPI确定生成。仅纯微信订单；福利卡账本、真实微信、staging/真机/production保持NOT_EXECUTED。',
  };
...
await updateCsv('04-P0-1至P0-119验收矩阵.csv', (row) => {
  ...
  if (row.P0ID === 'P0-024') return {
    ...
    LastVerifiedCommit: '', Verifier: 'CODEX', VerifiedAt: updatedAt,
...
await updateCsv('10-测试证据登记.csv', (row) => {
  ...
  if (row.EvidenceID === 'EVD-024') return {
    ...
    CommandOrProcedure: 'RED focused API；GREEN Prisma仓储3/3、Supertest 4/4、迁移契约1/1、P0 E2E1/1；prisma validate/migrate dry-run；OpenAPI generate/check/oasdiff',
    ...
    ExecutedAt: updatedAt, CommitSHA: '', CIRunURL: '',
...
await updateCsv('11-数据库迁移台账.csv', () => null, [{
  ...
  BackupRequired: 'YES', Status: 'LOCAL_PASS', AppliedLocalAt: updatedAt, AppliedStagingAt: '', AppliedProductionAt: '', CommitSHA: '',
...
await updateCsv(path.join('data', '阶段门禁.csv'), (row) => row.Stage === 'M3' ? {
  ...
  Notes: 'M3-P023已在main@fa8439f取得post-merge CI_PASS；M3-P024纯微信支付幂等本地focused与迁移/OpenAPI通过，PR/CI尚未创建；P0-024整体因福利卡和真实微信/staging保持NOT_EXECUTED；M3-P025及后续锁定。',
} : null);
...
status.github = {
  ...
  currentTaskDelivery: {
    ...
    branch: 'codex/m3-payment-idempotency', baseCommit: p023Merge, implementationCommit: null, deliveryHead: null,
  ...
  note: 'M3-P023已在main完成。M3-P024纯微信支付幂等本地focused/迁移/OpenAPI通过；福利卡账本及真实微信证据未进入本切片，P0-024整体保持NOT_EXECUTED；M3-P025锁定。',
};
status.evidence = {
  local: 'LOCAL_PASS_M3_P024_FOCUSED', ci: 'CI_PASS_M3_P023_MAIN',
