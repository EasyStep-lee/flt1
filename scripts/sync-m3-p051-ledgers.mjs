...
await updateCsv('11-数据库迁移台账.csv', (row) => row.MigrationID === 'MIG-014' ? { ...row,
  ...
  DataBackfill: 'NONE_NEW_TABLES_ONLY', Verification: 'Prisma validate；空库/升级/恢复/product drift dry-run；三资金来源/金额守恒/唯一键/历史不可变契约',
...
const apiRows = [
  { __key: 'API-101', ContractID: 'API-101', Stage: 'M3', Domain: 'company-welfare-card', Method: 'GET', Path: '/v1/company/welfare-card/programs', Actor: 'COMPANY_WELFARE_CARD', RequestDTO: 'None', ResponseDTO: 'WelfareProgramPageResponseDto', CommonResponse: '显式白名单DTO', ErrorCodes: 'AUTHENTICATION_REQUIRED|WORKSPACE_FORBIDDEN', Idempotency: 'NONE', SensitiveFieldPolicy: 'COMPANY_SESSION_DERIVED; never return companyId/enterpriseCustomerId/identityId/functionalAccountId/supplier fields', MoneyRule: 'integer cents only', P0: 'P0-051', OpenAPIStatus: 'GENERATED', DTOStatus: 'IMPLEMENTED', ErrorCodeStatus: 'IMPLEMENTED', ContractTest: 'apps/api/test/supertest/welfare-card-programs-api.test.mjs|packages/contracts/openapi.json', Owner: 'CODEX', Notes: 'private,no-store,noindex；仅当前公司福利卡职能。' },
  { __key: 'API-102', ContractID: 'API-102', Stage: 'M3', Domain: 'company-welfare-card', Method: 'POST', Path: '/v1/company/welfare-card/programs', Actor: 'COMPANY_WELFARE_CARD', RequestDTO: 'CreateWelfareProgramRequestDto', ResponseDTO: 'WelfareProgramResponseDto', CommonResponse: '显式白名单DTO', ErrorCodes: 'FIELD_FORBIDDEN|PERSONAL_RECHARGE_FORBIDDEN|WELFARE_FUNDING_SOURCE_INVALID|IDEMPOTENCY_KEY_REQUIRED|IDEMPOTENCY_CONFLICT|DUPLICATE_OR_STATE_CONFLICT', Idempotency: 'Idempotency-Key', SensitiveFieldPolicy: 'companyId/issuer/actor从会话及服务端派生；未知字段拒绝', MoneyRule: 'N/A', P0: 'P0-051', OpenAPIStatus: 'GENERATED', DTOStatus: 'IMPLEMENTED', ErrorCodeStatus: 'IMPLEMENTED', ContractTest: 'apps/api/test/supertest/welfare-card-programs-api.test.mjs|packages/contracts/openapi.json', Owner: 'CODEX', Notes: '资金来源仅三类；创建DRAFT并追加历史。' },
  { __key: 'API-103', ContractID: 'API-103', Stage: 'M3', Domain: 'company-welfare-card', Method: 'POST', Path: '/v1/company/welfare-card/programs/{programId}/batches', Actor: 'COMPANY_WELFARE_CARD', RequestDTO: 'CreateWelfareBatchRequestDto', ResponseDTO: 'WelfareBatchResponseDto', CommonResponse: '显式白名单DTO', ErrorCodes: 'WELFARE_PROGRAM_NOT_FOUND|WELFARE_BATCH_AMOUNT_MISMATCH|WELFARE_CLAIM_MODE_INVALID|IDEMPOTENCY_KEY_REQUIRED|IDEMPOTENCY_CONFLICT|DUPLICATE_OR_STATE_CONFLICT', Idempotency: 'Idempotency-Key', SensitiveFieldPolicy: 'company scope before lookup；response omits enterpriseCustomerId and all owner fields', MoneyRule: 'unitAmount*issueCount=totalAmount; safe positive integer cents', P0: 'P0-051', OpenAPIStatus: 'GENERATED', DTOStatus: 'IMPLEMENTED', ErrorCodeStatus: 'IMPLEMENTED', ContractTest: 'apps/api/test/supertest/welfare-card-programs-api.test.mjs|packages/contracts/openapi.json', Owner: 'CODEX', Notes: '领取方式必须匹配计划资金来源；创建DRAFT并追加历史。' },
];
