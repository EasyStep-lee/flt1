...
const migration = await readFile(
  new URL(
    '../../packages/db/prisma/migrations/20260810230500_m2_apparel_template_profile/migration.sql',
    import.meta.url,
  ),
  'utf8',
);
...
test('M2-P015 expands the existing profile constraint only through a forward migration', () => {
  assert.match(migration, /DROP CHECK `category_template_profile_check`/u);
  assert.match(
    migration,
    /CHECK \(`profile` IN \('GENERIC', 'FOOD', 'FRESH', 'APPAREL'\)\)/u,
  );
  assert.doesNotMatch(migration, /UPDATE `category_template`|DIGITAL|GIFT_BOX/iu);
});

test('M2-P015 retains the published profile and JSON immutability trigger', () => {
