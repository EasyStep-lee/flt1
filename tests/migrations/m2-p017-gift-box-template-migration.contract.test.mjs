...
const migration = await readFile(
  new URL(
    '../../packages/db/prisma/migrations/20260811090000_m2_gift_box_template_profile/migration.sql',
    import.meta.url,
  ),
  'utf8',
);
...
test('M2-P017 expands the existing profile constraint only through a forward migration', () => {
  assert.match(migration, /DROP CHECK `category_template_profile_check`/u);
  assert.match(
    migration,
    /CHECK \(`profile` IN \('GENERIC', 'FOOD', 'FRESH', 'APPAREL', 'DIGITAL', 'GIFT_BOX'\)\)/u,
  );
  assert.doesNotMatch(migration, /UPDATE `category_template`|DELETE|TRUNCATE/iu);
});

test('M2-P017 retains the published profile and JSON immutability trigger', () => {
