...
test('M1 ledgers retain the exact-head gate while M2 advances one slice at a time', async () => {
  ...
  const m2CompletedSlice = tasks.find(({ TaskID }) => TaskID === 'M2-P014');
  const m2CurrentSlice = tasks.find(({ TaskID }) => TaskID === 'M2-P015');
...
test('project status records M1 gate success while historical blocked handoff stays immutable', async () => {
  ...
  assert.equal(projectStatus.github.latestCi.headSha, m2p014Merge);
  ...
  assert.equal(projectStatus.github.previousTaskDelivery.exactHead, m2p014Head);
