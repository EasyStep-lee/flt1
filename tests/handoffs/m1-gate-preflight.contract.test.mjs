...
test('project status records M1 gate success while historical blocked handoff stays immutable', async () => {
  ...
  assert.equal(
    projectStatus.execution.prohibitedUntilGate.some((item) => /M3-P051.*M3-P052/u.test(item)),
    true,
  );
  assert.equal(
  ...
  assert.match(projectStatus.evidence.ci, /^(?:NOT_EXECUTED|CI_PASS_M3_P(?:03[01]|051)_HEAD_[0-9a-f]{7})$/u);
