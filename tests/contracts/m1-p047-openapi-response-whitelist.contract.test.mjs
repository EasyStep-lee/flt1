...
test('M1-P047 evidence remains closed after PR 20 merge as the project advances', async () => {
  ...
  assert.equal(state.github.pullRequest, 44);
  assert.equal(state.github.pullRequestState, 'MERGED');
  assert.equal(state.github.pullRequestMerged, true);
  assert.equal(state.github.mergeCommitSha, 'fb242c025673e937f63850f0677d7b0ffa61cdf4');
  assert.equal(state.github.currentTaskDelivery.taskId, 'M2-P010');
  ...
  assert.equal(state.github.previousTaskDelivery.taskId, 'M2-P009');
  assert.equal(state.github.previousTaskDelivery.pullRequest, 44);
