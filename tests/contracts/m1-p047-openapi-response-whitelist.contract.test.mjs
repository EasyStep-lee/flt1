...
test('M1-P047 evidence remains closed after PR 20 merge as the project advances', async () => {
  ...
  assert.equal(state.github.pullRequest, 50);
  ...
  assert.equal(state.github.currentTaskDelivery.taskId, 'M2-P012');
  ...
  assert.equal(state.github.previousTaskDelivery.taskId, 'M2-P011');
  assert.equal(state.github.previousTaskDelivery.pullRequest, 48);
