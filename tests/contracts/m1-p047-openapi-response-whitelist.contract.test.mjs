...
test('M1-P047 evidence remains closed after PR 20 merge as the project advances', async () => {
  ...
  assert.equal(state.execution.lastCompletedTask, 'M2-P006');
  ...
  assert.equal(state.github.pullRequest, 36);
  assert.equal(state.github.pullRequestState, 'MERGED');
  assert.equal(state.github.pullRequestMerged, true);
  assert.equal(state.github.mergeCommitSha, '2f52c218ab88f3a7419f2a77b7e6b730fd7365b6');
  assert.equal(state.github.currentTaskDelivery.taskId, 'M2-P006');
  ...
  assert.equal(state.github.previousTaskDelivery.taskId, 'M2-000');
  assert.equal(state.github.previousTaskDelivery.pullRequest, 36);
