...
test('all M2 ledgers are covered by codeable frozen contracts', async () => {
  ...
  assert.equal(frozenFields.length, 125);
  const implementedFieldRefinements = m2Fields.filter(
    ({ Status }) => Status === 'IMPLEMENTED_M2_P063',
  );
  assert.equal(implementedFieldRefinements.length, 18);
  assert.deepEqual(
    sort(frozenFields.map(({ entity, name }) => `${entity}.${name}`)),
    sort(
      m2Fields
        .filter(({ Status }) => Status !== 'IMPLEMENTED_M2_P063')
  ...
  assert.equal(freeze.stateContract.transitions.length, 12);
  assert.equal(
    m2States.filter(({ Status }) => Status === 'IMPLEMENTED_M2_P063').length,
    4,
  );
  ...
  assert.deepEqual(
    ...
    sort(m2ApiIds.filter((contractId) => !taskRefinements.some(({ ContractID }) => ContractID === contractId))),
  );
  ...
  assert.equal(
    taskRefinements.every(({ Notes }) => /任务内契约细化/u.test(Notes)),
    true,
  );
  assert.match(
...
test('machine control preserves the M2 freeze while later slices advance one gate at a time', async () => {
  ...
  const m2p063 = tasks.find(({ TaskID }) => TaskID === 'M2-P063');
  const m2p071 = tasks.find(({ TaskID }) => TaskID === 'M2-P071');
  ...
  assert.equal(m2p019.Status, 'DONE');
  ...
  assert.equal(m2p061.Status, 'DONE');
  assert.equal(m2p061.EvidenceStatus, 'CI_PASS');
  ...
  assert.equal(m2p061.PullRequest, 'https://github.com/EasyStep-lee/flt1/pull/68');
  assert.equal(m2p061.CI, 'CI_PASS');
  assert.equal(m2p063.Status, 'DONE');
  assert.equal(m2p063.EvidenceStatus, 'CI_PASS');
  assert.equal(m2p063.GitHubIssue, 'https://github.com/EasyStep-lee/flt1/issues/69');
  assert.equal(m2p063.Branch, 'codex/m2-shared-inventory-balance');
  assert.equal(m2p063.PullRequest, 'https://github.com/EasyStep-lee/flt1/pull/70');
  assert.equal(m2p063.CI, 'CI_PASS');
  assert.equal(m2p071.Status, 'DONE');
  assert.equal(m2p071.EvidenceStatus, 'CI_PASS');
  assert.equal(m2p071.GitHubIssue, 'https://github.com/EasyStep-lee/flt1/issues/71');
  assert.equal(m2p071.Branch, 'codex/m2-price-approval-pages');
  assert.equal(m2p071.PullRequest, 'https://github.com/EasyStep-lee/flt1/pull/72');
  ...
  assert.equal(projectStatus.execution.currentTask, 'M3-P055');
  assert.equal(projectStatus.execution.nextAllowedTask, 'M3-P055');
  assert.equal(projectStatus.execution.activeTaskCount, 1);
  assert.equal(projectStatus.execution.lastCompletedTask, 'M3-P054');
  assert.equal(projectStatus.execution.lastPassedGate, 'M2-GATE');
  assert.match(
    projectStatus.evidence.local,
    /^(?:LOCAL_FOCUSED_PASS_FULL_VERIFY_NOT_EXECUTED|LOCAL_PASS_M3_P030_FULL_VERIFY|LOCAL_PASS_M3_P031_FULL_VERIFY|LOCAL_PASS_M3_P051_FULL_VERIFY|LOCAL_PASS_M3_P052_FULL_VERIFY|LOCAL_PASS_M3_P053_FULL_VERIFY|LOCAL_PASS_M3_P054_FULL_VERIFY|LOCAL_PASS_M3_P055_FULL_VERIFY)$/u,
  );
