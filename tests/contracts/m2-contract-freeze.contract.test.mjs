...
test('machine control preserves the M2 freeze while later slices advance one gate at a time', async () => {
  ...
  assert.equal(projectStatus.execution.nextAllowedTask, 'M3-P052');
  ...
  assert.match(
    ...
    /^(?:LOCAL_FOCUSED_PASS_FULL_VERIFY_NOT_EXECUTED|LOCAL_PASS_M3_P030_FULL_VERIFY|LOCAL_PASS_M3_P031_FULL_VERIFY|LOCAL_PASS_M3_P051_FULL_VERIFY|LOCAL_PASS_M3_P052_FULL_VERIFY)$/u,
  );
  assert.match(projectStatus.evidence.ci, /^(?:NOT_EXECUTED|CI_PASS_M3_(?:P03[01]|P051)_HEAD_[0-9a-f]{7})$/u);
});
