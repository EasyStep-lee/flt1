...
const tree = { total: 3, items: [{
  id: rootId, parentId: null, name: '数码家电', level: 1, sortWeight: 10, status: 'ENABLED', version: 0,
  children: [{
    id: middleId, parentId: rootId, name: '办公数码', level: 2, sortWeight: 10, status: 'ENABLED', version: 0,
    children: [{
      id: leafId, parentId: middleId, name: '办公一体机', level: 3, sortWeight: 10,
...
test('P0-016 creates a DIGITAL draft with models, parameters, energy, package and company warranty', async ({ page }) => {
  ...
  await panel.getByRole('button', { name: '新建数码模板草稿' }).click();
  await expect.poll(() => submittedBody?.profile).toBe('DIGITAL');
  ...
  expect(body.fieldSchema.fields.map(({ key }) => key)).toEqual(expect.arrayContaining([
    'dimensions', 'power', 'voltage', 'interfaces', 'energy-efficiency', 'execution-standard',
    'package-list', 'installation-instructions', 'warranty-period',
  ]));
  expect(body.fieldSchema.fields).toContainEqual(expect.objectContaining({
    key: 'package-list', type: 'RICH_TEXT',
  }));
  expect(body.skuDimensions.dimensions.map(({ key }) => key)).toEqual(['color', 'capacity', 'model']);
  expect(body.detailModules.modules).toContainEqual(expect.objectContaining({
    key: 'digital-after-sales', kind: 'AFTER_SALE',
  }));
  expect(body.afterSaleRules).toMatchObject({
    returnPolicy: 'CATEGORY_RESTRICTED', notice: expect.stringContaining('江苏福礼团供应链科技有限公司'),
  });
  await expect(panel.getByText('数码', { exact: true })).toBeVisible();
